            ...
            <li class="nav-item">
                ...
                <div id="collapseUtilities" class="collapse show" aria-labelledby="headingUtilities" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item" href="ls-nb.php">Lịch sử nộp bài</a>
                    </div>
                </div>
            </li>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800"><strong>Thông tin bài tập</strong></h1>

                    <!-- Xử lý thông tin  -->
                    <?php
                    require_once('../../dao/bai-tap.php');
                    extract($_REQUEST);
                    $item = select_bai_tap_by_id($id_bt);
                    extract($item);

                    extract($_REQUEST);
                    if (array_key_exists("btn_submit", $_REQUEST)) {

                        if ($_FILES['nop_bai']['size'] == 0) {
                            echo '<script language="javascript">';
                            echo 'alert("Chưa đính kèm bài nộp!")';
                            echo '</script>';
                        } else {
                            $up_file = save_file("nop_bai", "attachments/");
                            $file = strlen($up_file) > 0 ? $up_file : 'attachment';
                            $username = $_SESSION['user']['username'];
                            date_default_timezone_set('Asia/Ho_Chi_Minh');
                            $thoi_gian_nop = date("Y-m-d H:i:s");

                            insert_nop_bai($id_bt, $username, $thoi_gian_nop, $file);
                            echo '<script language="javascript">';
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" value="<?= $ten_bt ?>" readonly>
                        </div>

                        <div class="form-group">
                            <label for="">Độ khó</label>
                            <input type="text" class="form-control" value="<?= $do_kho ?>" readonly>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" value="<?= $mo_ta ?>" readonly>
                        </div>

                        <div class="form-group">
                            <label for="">Nộp bài</label>
                            <input type="file" class="form-control-file" name="nop_bai">
                        </div>

                        <button type="submit" name="btn_submit" class="btn btn-primary custom">Nộp bài</button>
